(async function runV111Tests(globalScope) {
  ...
  if (isNode) {
    ...
    [appSource, indexSource, cssSource] = await Promise.all([
      fetch("../js/app.js?v=1.2.1-20260729a").then((response) => response.text()),
      fetch("../index.html?v=1.2.1-20260729a").then((response) => response.text()),
      fetch("../css/style.css?v=1.2.1-20260729a").then((response) => response.text())
    ]);
  }
